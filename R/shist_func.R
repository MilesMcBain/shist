...
#' This saves you having to replot over and over trying to find the bin width that best suits the inference you want to make.
...
shist <- function(x, bin_step=NA) {
  ...
  data_frame(var = x) %>%
      ggvis(~var) %>%
      layer_histograms(width =  input_slider(min=s_min, max=s_max, step = s_delta, label = "Bin Width"))
}
